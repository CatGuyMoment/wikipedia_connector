...
def hunt(start_page,end_page):
    ...
    while not response:
        ...
        if recursions >= 2:
            ...
            print(heuristic_scores[0],'score')
            data = [[heuristic_scores[0][0],heuristic_scores[0][2]]]
            heuristic_scores = []
            
        for page_name, path in tqdm(data):

            #ai scoring
            heuristic_scores.append([page_name,score_word_similarity(page_name,end_page),path])
...
def main():
    start = time.time()
    print(hunt('Sigmund_Freud','Institut_Florimont'))
